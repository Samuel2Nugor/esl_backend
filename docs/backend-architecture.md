...
The system currently uses two MQTT topics for communication between the backend and the gateway.

### esl/tag/write

purpose:
- send payload command from the backend to the gateway
...
Notes:
- The gateway validates the payload before processing it.
- The gateway then attempts BLE delivery to the ESL tag.

### esl/tag/ack

purpose:
- Send delivery acknowlegments and final delivery result back from the gateway
...
Notes:
- The ACk topic is used to report delivery status
- the backend is not yet subscribed to this topic

----

## Payload contract

The payload contract defines the required fields that the the backend must publish and the gateway must validate.
...
This contract mattes because both the backend and gateway must agree on the same message format.

## ACK contract

The ACK contract defines the delivery result message published by the gateway.
...
This contract matters because the backend will later use ACK messages to track whether a tag update was delivered successfully
